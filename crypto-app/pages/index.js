...
const Home = () => {
  ...
  const getTokensToBeClaimed = async () => {
    try {
      ...
      // invoke the nft contract -- get total number of minted MFTs
      ...
      const tokenContract = new Contract(
       ...
       // call the balanceOf to get total NFTs 
  ...
  const getBalanceOfCryptoDevTokens = async () => {
   ...
   try {
      ...
      // using the function to get the address
      const address = await signer.getAddress();

      //checking the balance using balanceOf
      const balance = await tokenContract.balanceOf(address);

      //set that Balance to state variable 
  ...
  const mintCryptoDevToken = async (amount) => {
   try {
      ...
      // 0.001 is the price for one CD token 
      const value = 0.001 * amount;

      // this tx send the amount of tokens and correspending eth in msg.value 
   ...
   */
   const getProviderOrSigner = async (needSigner = false) => {
    ...
    /*
      reutrns a button based on the current state 
   */
    ...
    const renderButton = () => {
         ...
         return (
            <div>
               <div className={styles.description}>
                  {tokensToBeClaimed * 10} Tokens to be clamied!
               </div>
      ...
      // we will give user miniting option once claiming is off
      return (
